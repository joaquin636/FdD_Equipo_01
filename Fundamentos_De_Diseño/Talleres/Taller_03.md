# BUSQUEDA BIBLIOGRAFICA

## Patentes

###  1. Wearable Navigation Assistance for the Vision-Impaired (US20140184384A1)

**Autores:** Zhu, Z., Ro, T., Ai, L., Khoo, W., Molina, E. & Palmer, F.  
**Año:** 2014

Esta patente propone un sistema de asistencia portátil compuesto por módulos sensor-actuador distribuidos en diferentes partes del cuerpo, como prendas, cinturones, muñecas, piernas y zapatos. Los sensores permiten detectar características del entorno, como la distancia hacia un obstáculo, mientras que los actuadores transforman esta información en estímulos perceptibles, principalmente mediante vibraciones. La propuesta permite que una persona con discapacidad visual reciba información sobre elementos que se encuentran en su recorrido sin depender de la visión, abordando una de las principales dificultades relacionadas con la seguridad y orientación durante el desplazamiento.

**Fuente:**  
https://patents.google.com/patent/US20140184384A1/en

<img width="1884" height="1529" alt="1" src="https://github.com/user-attachments/assets/24c3ab4f-429d-4ae7-a7cf-78e2e8e16dd5" />

---

### 2. Wearable Navigation System for the Visually Impaired (US10900788B2)

**Autor:** Anantha, S.  
**Año:** 2021

Esta patente presenta un sistema wearable que combina sensores de distancia, cámaras y procesamiento de imágenes para obtener información sobre el entorno. Los dispositivos de medición pueden distribuirse sobre el cuerpo para detectar elementos desde diferentes posiciones, mientras que las cámaras permiten identificar objetos presentes en el campo de visión. La información obtenida se transforma en estímulos no visuales para el usuario, proporcionando una alternativa para reconocer elementos del entorno que podrían dificultar el recorrido y apoyar la orientación y seguridad de las personas con discapacidad visual.

**Fuente:**  
https://patents.google.com/patent/US10900788B2/en

<img width="2043" height="1610" alt="US10900788-20210126-D00002" src="https://github.com/user-attachments/assets/041fc1aa-49e9-4939-ab87-e70a354d0a68" />

---

### 3. Stereophonic Apparatus for Blind and Visually-Impaired People (EP3924873A1)

**Autores:** Solomon, L. & Solomon, D.  
**Año:** 2021 

Esta patente plantea un sistema wearable de asistencia que integra sensores de distancia, cámaras, dispositivos de audio y procesamiento mediante algoritmos e inteligencia artificial. Los datos obtenidos del entorno pueden ser procesados mediante dispositivos portátiles, como un teléfono inteligente o una Raspberry Pi, y utilizados para proporcionar información al usuario. La propuesta considera diferentes formas de captar y comunicar información ambiental, lo que permite abordar las dificultades que una persona con discapacidad visual puede encontrar al identificar elementos de su entorno durante sus desplazamientos.

**Fuente:**  
https://patents.google.com/patent/EP3924873A1/en


---

## Tesis

### 4. Diseño de dispositivo basado en ultrasonido para desplazamiento de personas en condición de discapacidad visual

**Autor:** Magno Parra Farfán  
**Año:** 2015

Esta investigación desarrolla un dispositivo de asistencia basado en ultrasonido para apoyar el desplazamiento de personas con discapacidad visual. El sistema permite detectar obstáculos y estimar su posición a distancias de hasta 2,50 m. El trabajo muestra cómo la detección de elementos presentes en el entorno puede ampliar la percepción de una persona que no puede identificarlos visualmente, contribuyendo a comprender las necesidades relacionadas con la seguridad y movilidad durante los recorridos cotidianos.

**Fuente:**  
https://tesis.pucp.edu.pe/items/1b43d841-9f9f-4789-8804-75b69a36710f

---

### 5. Bastón sensorial geolocalizador inteligente para apoyar en el desplazamiento de personas invidentes

**Autor:** Roger Jaime Fernández Llontop  
**Año:** 2021

Esta investigación desarrolla un bastón sensorial con geolocalización para apoyar el desplazamiento de personas con discapacidad visual en Chiclayo. La propuesta integra el bastón tradicional con tecnologías de orientación, localización y asistencia, ampliando sus funciones durante el recorrido. El trabajo permite considerar que las dificultades de movilidad no se relacionan únicamente con la presencia de obstáculos, sino también con la necesidad de conocer la ubicación y orientarse adecuadamente para desplazarse de manera más autónoma.

**Fuente:**  
https://repositorio.usat.edu.pe/items/38a8bb81-5684-4b5b-95bc-2445624bf977/full

---

### 6. Diseño e implementación de un prototipo de ubicación espacial para personas con discapacidad visual mediante IoT

**Autor:** Frank Anthony Cayambe Gamarra

Esta investigación propone un dispositivo corporal basado en IoT para proporcionar información espacial a personas con discapacidad visual. El prototipo integra sensores ultrasónicos, sensores de proximidad y GPS para obtener información relacionada tanto con el entorno como con la ubicación del usuario. Estos datos son comunicados en tiempo real mediante señales táctiles y de audio. La propuesta reúne detección de obstáculos, ubicación y comunicación de información, abordando diferentes necesidades que pueden presentarse durante el desplazamiento y la orientación en espacios cotidianos.

**Fuente:**  
https://dspace.ups.edu.ec/handle/123456789/30245

---

## Artículos de divulgación e institucionales

### 7. Blindness and vision impairment

**Institución:** Organización Mundial de la Salud (OMS)

La Organización Mundial de la Salud señala que al menos 2.200 millones de personas presentan algún tipo de deficiencia visual, de las cuales al menos 1.000 millones podrían haberse prevenido o aún no han sido atendidas adecuadamente. Esta información permite dimensionar la magnitud de la discapacidad visual y la importancia de considerar las dificultades que pueden presentarse en las actividades cotidianas. Dentro de estas dificultades se encuentran aquellas relacionadas con la orientación, movilidad y desplazamiento seguro, aspectos que pueden influir en la autonomía y participación de las personas en diferentes espacios.

**Fuente:**  
https://www.who.int/news-room/fact-sheets/detail/blindness-and-visual-impairment

---

### 8. Stanford Researchers Build $400 Self-Navigating Smart Cane

**Institución:** Stanford University

Stanford presenta un bastón aumentado que combina el bastón convencional con LiDAR, GPS, sensores e inteligencia artificial para detectar obstáculos y orientar al usuario. Además, incorpora algoritmos de navegación y una rueda motorizada que proporciona asistencia durante el desplazamiento. En las pruebas, los participantes caminaron aproximadamente un 20 % más rápido que utilizando un bastón blanco convencional. Este proyecto evidencia cómo la integración de detección, navegación y asistencia física puede contribuir a enfrentar dificultades de movilidad y favorecer desplazamientos más autónomos.

**Fuente:**  
https://hai.stanford.edu/news/stanford-researchers-build-400-self-navigating-smart-cane

---

### 9. Proyecto de bastón inteligente de estudiantes UARM

**Institución:** Universidad Antonio Ruiz de Montoya (UARM)

Estudiantes de Ingeniería Industrial de la UARM desarrollaron un bastón inteligente de bajo costo para apoyar la movilidad de personas con discapacidad visual. El prototipo utiliza un sensor de proximidad para detectar obstáculos aproximadamente a 60 cm y comunica su presencia mediante sonidos y vibraciones. También incorpora un sensor de inclinación para identificar cuando el bastón ha caído. La propuesta muestra una alternativa accesible que combina detección del entorno y alertas al usuario, elementos relacionados con la necesidad de desplazarse con mayor seguridad y autonomía.

**Fuente:**  
https://www.uarm.edu.pe/noticia/exitosa-destaca-proyecto-de-ingenieria-industrial-uarm-para-personas-con-discapacidad-visual/

---

## Artículos científicos

### 10. Obstacle Detection Display for Visually Impaired: Coding of Direction, Distance, and Height on a Vibrotactile Waist Band

**Autores:** van Erp, Kroon, Mioch & Paul  
**Año:** 2017

Este estudio analiza un cinturón vibrotáctil diseñado para transmitir información sobre la dirección, distancia y altura de los obstáculos mediante diferentes patrones de vibración. La investigación muestra cómo las características de los elementos presentes en el entorno pueden comunicarse mediante el sentido del tacto, permitiendo que el usuario obtenga información relevante para orientar su desplazamiento sin depender de la visión. Además, señala que transmitir demasiada información al mismo tiempo puede generar sobrecarga cognitiva, aspecto importante al diseñar sistemas de asistencia para una movilidad segura y comprensible.

**Fuente:**  
https://www.frontiersin.org/journals/ict/articles/10.3389/fict.2017.00023/full

---

### 11. A Wearable Assistive Device for Blind Pedestrians Using Real-Time Object Detection and Tactile Presentation

**Autores:** Shen, Chen & Sawada  
**Año:** 2022

Esta investigación propone un dispositivo wearable para peatones con discapacidad visual que combina la detección de objetos en tiempo real con señales táctiles. El sistema busca identificar elementos presentes durante el desplazamiento y comunicar esta información mediante estímulos que puedan ser percibidos por el usuario. Además, plantea una solución portátil que puede procesar información utilizando hardware con recursos limitados. El estudio permite observar cómo la identificación de objetos y la comunicación táctil pueden utilizarse para apoyar la percepción del entorno y reducir las dificultades relacionadas con la movilidad segura.

**Fuente:**  
https://pubmed.ncbi.nlm.nih.gov/35746319/

---

### 12. GuideTouch: An Obstacle Avoidance Device for Visually Impaired

**Autores:** Kozlov et al.  
**Año:** 2026

GuideTouch presenta un dispositivo de asistencia que utiliza sensores de distancia y actuadores vibrotáctiles ubicados en la parte superior del cuerpo. Su principal aporte consiste en detectar obstáculos elevados que podrían no ser identificados mediante un bastón convencional. Esta característica permite reconocer que las dificultades de desplazamiento de las personas con discapacidad visual no se limitan a los obstáculos ubicados a nivel del suelo, sino que también pueden incluir elementos situados a diferentes alturas. Por ello, el estudio amplía la comprensión de los riesgos que pueden presentarse durante el desplazamiento cotidiano.

**Fuente:**  
https://www.researchgate.net/publication/399956302_GuideTouch_An_Obstacle_Avoidance_Device_for_Visually_Impaired

---

## Productos comerciales

### 13. WeWALK Smart Cane 2

WeWALK Smart Cane 2 mantiene las funciones de un bastón convencional e incorpora herramientas tecnológicas para la detección de obstáculos, navegación y asistencia mediante voz e inteligencia artificial. La combinación de las funciones tradicionales con herramientas de orientación permite proporcionar información adicional al usuario durante sus desplazamientos. Este producto muestra cómo diferentes tecnologías pueden integrarse en un dispositivo de movilidad para responder a necesidades relacionadas con la percepción del entorno, orientación y autonomía.

**Fuente:**  
https://ifdesign.com/es/winner-ranking/project/wewalk-smart-cane-2/676885

<img width="589" height="1000" alt="wewalk_1" src="https://github.com/user-attachments/assets/bcfad86f-b389-467a-899e-54926c9049fe" />

---

### 14. NOA 

NOA es un dispositivo wearable en forma de chaleco o arnés que incorpora cámaras, inteligencia artificial y guía sonora para ayudar al usuario a detectar obstáculos y orientarse durante sus desplazamientos. El sistema utiliza las cámaras para obtener información del entorno y proporciona indicaciones sonoras al usuario. De esta manera, plantea una alternativa para acceder a información ambiental sin depender directamente de la percepción visual, abordando dificultades de orientación y desplazamiento que pueden limitar la autonomía de las personas con discapacidad visual.

**Fuente:**

https://biped.ai/

<img width="435" height="731" alt="WhatsApp Image 2026-09-02 at 12 21 32 PM" src="https://github.com/user-attachments/assets/0da2cd4a-c4dd-4928-80e4-7f5ef135fa40" />

---

### 15. Glide 

Glide es un dispositivo de asistencia a la movilidad que combina cámaras y sensores para detectar obstáculos durante el desplazamiento. Además de identificar los elementos presentes en el recorrido, el dispositivo proporciona guía física al usuario para desplazarse alrededor de ellos. Esta característica permite observar una propuesta que no se limita a informar sobre los obstáculos, sino que también busca facilitar el recorrido, relacionándose directamente con las dificultades de movilidad y seguridad que pueden experimentar las personas con discapacidad visual en sus desplazamientos cotidianos.

**Fuente:**  
https://www.glidance.io/

<img width="1920" height="1238" alt="6a463e344281bc58b993cc77_manwalking2_1" src="https://github.com/user-attachments/assets/0a234c21-8576-4287-bcba-b30f7b78dcf2" />

---
# Matriz de antecedentes

| N.° | Referencia | Tipo de documento | ¿En qué aporta a nuestro proyecto? | Información clave e importante | Valores de medición / variables | Imagen |
|---|---|---|---|---|---|---|
| **1** | **Zhu et al. (2014). Wearable Navigation Assistance for the Vision-Impaired** | Patente | Demuestra que es posible colocar múltiples sensores y actuadores hápticos en diferentes partes del cuerpo para representar espacialmente los obstáculos. Es un antecedente directo de nuestra idea de retroalimentación háptica espacial. | Utiliza pares sensor-actuador en brazos, ropa, cinturón, cabeza, piernas o zapatos. La vibración cambia según la proximidad del obstáculo. Los sensores pueden distribuirse alrededor del usuario. | **Mide:** distancia/proximidad al obstáculo. **Salida:** intensidad de vibración. Puede plantearse una cobertura cercana a **360°** mediante varios sensores. No se especifica un rango exacto de detección. | ![Imagen](imagenes/zhu2014.png) |
| **2** | **Anantha (2021). Wearable Navigation System for the Visually Impaired** | Patente | Aporta el concepto de combinar varios tipos de sensores y cámaras para obtener información más completa del entorno. Se relaciona con nuestra propuesta de integrar varias fuentes de información antes de determinar el peligro. | Combina sensores de distancia, cámaras y reconocimiento de imágenes. Puede utilizar varios sensores de distancia para identificar la ubicación y distancia de objetos. | **Mide:** distancia, dirección, posición e información sobre objetos. No se identificó un rango numérico específico. | ![Imagen](imagenes/anantha2021.png) |
| **3** | **Solomon & Solomon (2021). Stereophonic Apparatus for Blind and Visually-Impaired People** | Patente | Aporta el uso combinado de cámaras, sensores de distancia, GPS e IA para comprender el contexto del entorno y no limitarse únicamente a detectar proximidad. | Puede utilizar LiDAR o ultrasonido junto con cámaras. Reconoce calles, aceras, vehículos, semáforos, puertas y ascensores, y adapta su comportamiento según el escenario. | **Mide/estima:** distancia, posición, localización GPS y reconocimiento de elementos del entorno. No se especifican valores numéricos concretos. | ![Imagen](imagenes/solomon2021.png) |
| **4** | **Hands-Free Crowd Sourced Indoor Navigation System and Method – US20230050825A1** | Patente | Aporta una aproximación a la navegación inteligente en interiores y demuestra que la información obtenida de sensores puede complementarse con mapas, procesamiento y otras fuentes. | Está orientada a ayudar a personas con discapacidad visual en navegación interior sin depender exclusivamente del GPS. Utiliza información del entorno para orientar al usuario. | **Mide/estima:** posición, orientación y ruta dentro de espacios interiores. No se encontraron valores cuantitativos relevantes para nuestra comparación inicial. | ![Imagen](imagenes/navigation2023.png) |
| **5** | **Parra Farfán (2015). Diseño de dispositivo basado en ultrasonido para desplazamiento de personas en condición de discapacidad visual** | Tesis | Es un antecedente directo del uso de ultrasonido como tecnología económica para detectar obstáculos. Ayuda a justificar el uso de sensores de distancia en un prototipo. | Diseña un dispositivo capaz de detectar obstáculos y estimar aproximadamente dónde se encuentran. | **Mide:** distancia al obstáculo. **Rango reportado:** aproximadamente **2,50 m**. | ![Imagen](imagenes/parra2015.png) |
| **6** | **Fernández Llontop (2021). Bastón sensorial geolocalizador inteligente** | Tesis | Aporta evidencia peruana de que es posible combinar detección de obstáculos y geolocalización en una ayuda tecnológica para personas con discapacidad visual. | Integra un bastón sensorial con detección y geolocalización para apoyar la movilidad de usuarios con discapacidad visual en Chiclayo. | **Mide/estima:** proximidad a obstáculos y ubicación geográfica. No se especificó un rango de detección concreto en la información recopilada. | ![Imagen](imagenes/fernandez2021.png) |
| **7** | **Cayambe Gamarra. Diseño e implementación de un prototipo de ubicación espacial para personas con discapacidad visual mediante IoT** | Tesis | Aporta la integración de tecnologías IoT dentro de dispositivos destinados a mejorar la ubicación espacial y movilidad. Puede servir como antecedente para la comunicación entre sensores, procesamiento y otros dispositivos. | Desarrolla un prototipo de ubicación espacial utilizando tecnologías IoT para personas con discapacidad visual. | **Variables:** ubicación espacial, información obtenida mediante sensores y comunicación IoT. No se identificó un valor cuantitativo específico. | ![Imagen](imagenes/cayambe.png) |
| **8** | **World Health Organization. Blindness and Vision Impairment** | Documento institucional / ficha informativa | Justifica la magnitud social del problema y permite fundamentar por qué la movilidad y autonomía de las personas con discapacidad visual requieren soluciones tecnológicas. | La discapacidad visual puede afectar la movilidad, participación social, educación, empleo y calidad de vida. Es útil para relacionar el proyecto con el **ODS 10 y la Meta 10.2**. | **Dato epidemiológico:** al menos **2 200 millones de personas** en el mundo presentan deficiencia visual cercana o lejana. No evalúa un dispositivo. | ![Imagen](imagenes/oms.png) |
| **9** | **Stanford Researchers Build $400 Self-Navigating Smart Cane** | Artículo institucional / divulgación científica | Demuestra que tecnologías provenientes de robótica y vehículos autónomos pueden incorporarse a ayudas de movilidad. Pasa de simplemente detectar obstáculos a ayudar al usuario a evitarlos. | Bastón robotizado con sensores y navegación capaz de orientar al usuario durante el desplazamiento. | Los participantes caminaron aproximadamente **20 % más rápido** con el bastón aumentado que con un bastón blanco convencional. **Costo aproximado:** **US$400**. | ![Imagen](imagenes/stanford-cane.png) |
| **10** | **Proyecto de bastón inteligente UARM** | Artículo institucional / proyecto tecnológico | Es un antecedente peruano de bajo costo y demuestra que sensores sencillos pueden utilizarse para construir dispositivos de asistencia accesibles. | Utiliza sensores de proximidad y alertas mediante sonido y vibración para advertir sobre obstáculos. | **Distancia de detección:** aproximadamente **60 cm**. **Costo aproximado:** alrededor de **S/120**. | ![Imagen](imagenes/uarm.png) |
| **11** | **van Erp et al. (2017). Obstacle Detection Display for Visually Impaired** | Artículo científico experimental | Es uno de los antecedentes más importantes para nuestra propuesta porque demuestra que la retroalimentación vibrotáctil funciona, pero también que transmitir demasiada información puede generar sobrecarga. Justifica que nuestro sistema deba priorizar riesgos. | Utiliza un cinturón vibrotáctil que comunica dirección, distancia y altura mediante patrones táctiles. Los experimentos muestran que debe controlarse la cantidad de información enviada al usuario. | **Variables estudiadas:** dirección, distancia, altura de obstáculos, reconocimiento de patrones táctiles y carga informativa. | ![Imagen](imagenes/vanerp2017.png) |
| **12** | **Shen, Chen & Sawada (2022). A Wearable Assistive Device for Blind Pedestrians Using Real-Time Object Detection and Tactile Presentation** | Artículo científico experimental | Aporta evidencia de que la detección de objetos mediante IA puede integrarse en hardware portátil y combinarse con retroalimentación táctil. Es relevante para el futuro reconocimiento y seguimiento de objetos del proyecto. | Utiliza detección de objetos en tiempo real y actuadores táctiles. La red neuronal fue optimizada para ejecutarse en dispositivos con recursos limitados. | Se reporta una reducción aproximada del **90 % de estructuras redundantes de la red neuronal**. También se evalúa detección y presentación táctil. | ![Imagen](imagenes/shen2022.png) |
| **13** | **Kozlov et al. (2026). GuideTouch: An Obstacle Avoidance Device for Visually Impaired** | Artículo científico / publicación HRI-ACM | Es uno de los antecedentes tecnológicos más cercanos al proyecto. Demuestra que sensores ToF y actuadores distribuidos sobre el cuerpo pueden proporcionar información direccional con alta precisión. | Utiliza **2 sensores Time-of-Flight** colocados verticalmente y **4 actuadores vibrotáctiles** distribuidos entre hombros y pecho. Está orientado especialmente a obstáculos elevados. | **22 participantes:** aproximadamente **92,9 % de precisión media** para señales direccionales principales. **14 usuarios con discapacidad visual:** aproximadamente **93,75 %** para las señales principales. | ![Imagen](imagenes/guidetouch.png) |
| **14** | **WeWALK Smart Cane 2** | Producto comercial | Demuestra que existe un mercado real para tecnologías que combinan el bastón tradicional con sensores, navegación e IA. Permite identificar funciones que ya existen comercialmente y que no deberían presentarse como innovación propia. | Incorpora detección de obstáculos, navegación y funciones de voz e inteligencia artificial. Continúa funcionando como bastón convencional incluso sin conexión. | **Variables:** proximidad de obstáculos y navegación. No se especificó un valor experimental comparable de precisión o rango en las fuentes revisadas. | ![Imagen](imagenes/wewalk.png) |
| **15** | **NOA – biped.ai** | Producto comercial | Es uno de los competidores más próximos conceptualmente, porque utiliza IA y sensores para interpretar el entorno y detectar incluso objetos móviles. Ayuda a diferenciar nuestro proyecto mediante la estimación y priorización del riesgo de colisión. | Wearable colocado sobre los hombros con cámaras, inteligencia artificial, navegación y guía mediante sonido espacial o conducción ósea. | Se han descrito **3 cámaras de amplio ángulo**. Analiza obstáculos, profundidad y elementos del entorno. No se proporciona una precisión experimental comparable en la información recopilada. | ![Imagen](imagenes/noa-biped.png) |
| **16** | **Glide – Glidance** | Producto comercial | Demuestra que una ayuda tecnológica puede ir más allá de advertir obstáculos y realizar acciones para evitarlos. Sirve para comparar nuestra decisión de mantener al usuario en control mediante información háptica en lugar de dirección física automatizada. | Dispositivo de movilidad con ruedas, cámaras, sensores e IA. Detecta obstáculos y guía físicamente al usuario alrededor de ellos. | **Hardware:** sistema de **2 ruedas**, cámaras y sensores. **Variables:** profundidad, obstáculos, trayectoria y navegación. No se proporcionan valores cuantitativos de precisión en la información recopilada. | ![Imagen](imagenes/glide.png) |

## Variables que podría utilizar nuestro proyecto

| Variable | ¿Qué representa? | Unidad / clasificación | Ejemplo |
|---|---|---|---|
| **Distancia** | Separación entre el usuario y un objeto. | Metros (m) | Persona a **2,5 m** |
| **Dirección** | Lugar desde donde se encuentra o aproxima el objeto. | Grados (°) o sectores | Izquierda, derecha, frente o atrás |
| **Velocidad relativa** | Qué tan rápido se acerca o aleja un objeto respecto al usuario. | Metros por segundo (m/s) | Persona acercándose a **1,2 m/s** |
| **Trayectoria** | Dirección de movimiento estimada del objeto. | Coordenadas / vector de movimiento | Persona cruzando de izquierda a derecha |
| **Tiempo hasta colisión (TTC)** | Tiempo estimado antes de que ocurra una posible colisión si se mantienen las trayectorias actuales. | Segundos (s) | **1,8 s** |
| **Nivel de riesgo** | Prioridad asignada a cada obstáculo según su posibilidad de producir una colisión. | Bajo / medio / alto | **Alto** |
| **Dirección háptica** | Zona corporal donde se genera la alerta para indicar la dirección del peligro. | Zona del wearable | Hombro izquierdo |
| **Intensidad o frecuencia de vibración** | Nivel de urgencia comunicado al usuario. | Intensidad / frecuencia | Vibración rápida = peligro inmediato |

## Interpretación

Los antecedentes muestran una evolución desde sistemas que únicamente detectan la **proximidad de obstáculos** hasta dispositivos capaces de realizar **reconocimiento de objetos, navegación inteligente y evitación automática**.

Nuestra propuesta busca avanzar hacia un sistema que siga el siguiente proceso:

**Detección → Seguimiento → Predicción → Evaluación de riesgo → Priorización → Alerta háptica**

El objetivo principal no sería advertir al usuario sobre todos los objetos presentes en su entorno, sino identificar cuáles representan realmente un **riesgo potencial de colisión**.

Por ejemplo:

- Pared a **1,5 m** a la derecha → riesgo bajo si el usuario no se dirige hacia ella.
- Persona quieta a **2 m** delante → riesgo medio.
- Persona acercándose rápidamente desde la izquierda → **riesgo alto**.
- Persona caminando detrás en la misma dirección → riesgo bajo.

En este escenario, el dispositivo debería priorizar únicamente la alerta correspondiente al objeto que se aproxima desde la izquierda, evitando generar múltiples vibraciones innecesarias que puedan saturar al usuario.





